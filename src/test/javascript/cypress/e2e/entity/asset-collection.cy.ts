...
describe('AssetCollection e2e test', () => {
  ...
  const assetCollectionSample = { name: 'sole outperform', activationStatus: 'PENDENT' };
  ...
  describe('new AssetCollection page', () => {
    ...
    it('should create an instance of AssetCollection', () => {
      cy.get(`[data-cy="name"]`).type('ugh along yahoo');
      cy.get(`[data-cy="name"]`).should('have.value', 'ugh along yahoo');

      cy.get(`[data-cy="fullFilenamePath"]`).type('for');
      cy.get(`[data-cy="fullFilenamePath"]`).should('have.value', 'for');
