...
describe('CustomerType e2e test', () => {
  ...
  const customerTypeSample = { name: 'readily till' };
  ...
  describe('new CustomerType page', () => {
    ...
    it('should create an instance of CustomerType', () => {
      cy.get(`[data-cy="name"]`).type('which');
      cy.get(`[data-cy="name"]`).should('have.value', 'which');

      cy.get(`[data-cy="description"]`).type('comfortable');
      cy.get(`[data-cy="description"]`).should('have.value', 'comfortable');

      cy.get(`[data-cy="businessHandlerClazz"]`).type('brushing chain');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'brushing chain');
