...
describe('TownCity e2e test', () => {
  ...
  // const townCitySample = {"acronym":"endeavor","name":"foolish"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/provinces',
      body: {"acronym":"ser","name":"fussy bolero","geoPolygonArea":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","geoPolygonAreaContentType":"unknown"},
    }).then(({ body }) => {
      province = body;
    });
  });
  ...
  describe('new TownCity page', () => {
    ...
    it.skip('should create an instance of TownCity', () => {
      cy.get(`[data-cy="acronym"]`).type('total no');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'total no');

      cy.get(`[data-cy="name"]`).type('boo dissimulate');
      cy.get(`[data-cy="name"]`).should('have.value', 'boo dissimulate');
