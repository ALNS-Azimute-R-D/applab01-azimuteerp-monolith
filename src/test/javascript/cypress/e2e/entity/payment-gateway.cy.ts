...
describe('PaymentGateway e2e test', () => {
  ...
  const paymentGatewaySample = { aliasCode: 'steal muscatel', description: 'annually duh', activationStatus: 'INVALID' };
  ...
  describe('new PaymentGateway page', () => {
    ...
    it('should create an instance of PaymentGateway', () => {
      cy.get(`[data-cy="aliasCode"]`).type('scaly inhabit that');
      cy.get(`[data-cy="aliasCode"]`).should('have.value', 'scaly inhabit that');

      cy.get(`[data-cy="description"]`).type('latter versus keenly');
      cy.get(`[data-cy="description"]`).should('have.value', 'latter versus keenly');

      cy.get(`[data-cy="businessHandlerClazz"]`).type('meanwhile');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'meanwhile');
