...
describe('CommonLocality e2e test', () => {
  ...
  // const commonLocalitySample = {"acronym":"kookily","name":"elated half whoever","streetAddress":"off","postalCode":"amongst m"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/districts',
      body: {"acronym":"upright ","name":"eggplant front","geoPolygonArea":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","geoPolygonAreaContentType":"unknown"},
    }).then(({ body }) => {
      district = body;
    });
  });
  ...
  describe('new CommonLocality page', () => {
    ...
    it.skip('should create an instance of CommonLocality', () => {
      cy.get(`[data-cy="acronym"]`).type('remaster masculine');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'remaster masculine');

      cy.get(`[data-cy="name"]`).type('lunge female');
      cy.get(`[data-cy="name"]`).should('have.value', 'lunge female');

      cy.get(`[data-cy="description"]`).type('curly extraneous dearest');
      cy.get(`[data-cy="description"]`).should('have.value', 'curly extraneous dearest');

      cy.get(`[data-cy="streetAddress"]`).type('although whose');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'although whose');

      cy.get(`[data-cy="houseNumber"]`).type('acclaimed');
      cy.get(`[data-cy="houseNumber"]`).should('have.value', 'acclaimed');

      cy.get(`[data-cy="locationName"]`).type('vivaciously phew');
      cy.get(`[data-cy="locationName"]`).should('have.value', 'vivaciously phew');

      cy.get(`[data-cy="postalCode"]`).type('excepting');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'excepting');
