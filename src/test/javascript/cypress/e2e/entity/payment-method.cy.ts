...
describe('PaymentMethod e2e test', () => {
  ...
  const paymentMethodSample = { code: 'gainsay gl', description: 'ruling while save' };
  ...
  describe('new PaymentMethod page', () => {
    ...
    it('should create an instance of PaymentMethod', () => {
      cy.get(`[data-cy="code"]`).type('feline wor');
      cy.get(`[data-cy="code"]`).should('have.value', 'feline wor');

      cy.get(`[data-cy="description"]`).type('hasty under justly');
      cy.get(`[data-cy="description"]`).should('have.value', 'hasty under justly');

      cy.get(`[data-cy="businessHandlerClazz"]`).type('pfft till');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'pfft till');
