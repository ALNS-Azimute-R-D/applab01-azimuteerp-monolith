...
public class SupplierRepositoryWithBagRelationshipsImpl implements SupplierRepositoryWithBagRelationships {
    ...
    public Optional<Supplier> fetchBagRelationships(Optional<Supplier> supplier) {
        return supplier.map(this::fetchProductsLists);
    }
    ...
    public List<Supplier> fetchBagRelationships(List<Supplier> suppliers) {
        return Optional.of(suppliers).map(this::fetchProductsLists).orElse(Collections.emptyList());
    }

    Supplier fetchProductsLists(Supplier result) {
        return entityManager
            .createQuery(
                "select supplier from Supplier supplier left join fetch supplier.productsLists where supplier.id = :id",
                Supplier.class
            )
    ...
    List<Supplier> fetchProductsLists(List<Supplier> suppliers) {
        ...
        List<Supplier> result = entityManager
            .createQuery(
                "select supplier from Supplier supplier left join fetch supplier.productsLists where supplier in :suppliers",
                Supplier.class
            )
