...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(
        value = "select customer from Customer customer left join fetch customer.customerType left join fetch customer.district",
        countQuery = "select count(customer) from Customer customer"
    )
    Page<Customer> findAllWithToOneRelationships(Pageable pageable);

    @Query("select customer from Customer customer left join fetch customer.customerType left join fetch customer.district")
    List<Customer> findAllWithToOneRelationships();

    @Query(
        "select customer from Customer customer left join fetch customer.customerType left join fetch customer.district where customer.id =:id"
    )
